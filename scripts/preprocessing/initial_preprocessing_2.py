...
class BiosignalPreprocessor:
    ...
    def __init__(self, window_size: int = 60, min_data_threshold: float = 0.6):
        """
    ...
    def _calculate_hrv_features(self, ibi_series: pd.Series) -> Dict:
        """
        Calculate time-domain, frequency-domain, and non-linear HRV features.
        """
        ibi_values_sec = ibi_series.values
        
        # --- Time-Domain Features (existing) ---
        features = {}
        if len(ibi_values_sec) >= 2:
            diff_ibi = np.diff(ibi_values_sec)
            features['hrv_mean_ibi'] = np.mean(ibi_values_sec)
            features['hrv_sdnn'] = np.std(ibi_values_sec)
            features['hrv_rmssd'] = np.sqrt(np.mean(diff_ibi ** 2))
            features['hrv_pnn50'] = (np.abs(diff_ibi) > 0.05).sum() / len(diff_ibi) * 100
        else:
            features.update({
                'hrv_mean_ibi': np.nan,
                'hrv_sdnn': np.nan,
        ...
        # --- Frequency-Domain and Non-Linear Features (new) ---
        # Convert IBI from seconds to milliseconds for pyHRV
        ibi_values_ms = ibi_values_sec * 1000
        
        try:
            # Frequency-domain features using Welch's method
            freq_results = welch_psd(nni=ibi_values_ms, show=False, mode='dev')
            
            # Non-linear Poincaré plot features
            ...
            features.update({
                'hrv_lf': freq_results['lf'],
                'hrv_hf': freq_results['hf'],
                'hrv_lf_hf_ratio': freq_results['lf_hf_ratio'],
                'hrv_sd1': poincare_results['sd1'],
                'hrv_sd2': poincare_results['sd2'],
                'hrv_sd1_sd2_ratio': poincare_results['sd1_sd2_ratio'],
                'hrv_sampen': sampen_results['sampen']
            })

        except Exception as e:
            logging.warning(f"HRV feature calculation failed for window (likely too short or invalid data): {e}")
