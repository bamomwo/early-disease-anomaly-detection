...
def resample_sensor_data(df: pd.DataFrame, sensor: str, target_fs: float) -> pd.DataFrame:
    ...
    try:
        ...
        # Calculate resampling rule (period)
        period = f"{int(1000/target_fs)}ms"  # Convert Hz to milliseconds
        
        if sensor == "IBI":
            # For IBI, interpolate irregular data to regular intervals
            df_resampled = df_resampled.resample(period).mean().interpolate(method='linear')
        else:
            # For regular sensors, use appropriate aggregation
            if sensor == "ACC":
                # For accelerometer, keep all axes
                df_resampled = df_resampled.resample(period).mean()
            else:
                # For other sensors, use mean aggregation
                df_resampled = df_resampled.resample(period).mean()
        
        # Reset index to get timestamp back as column
        df_resampled = df_resampled.reset_index()
        
        # Remove NaN rows that might result from resampling
        df_resampled = df_resampled.dropna()
        
        logging.debug(f"Resampled {sensor} from {len(df)} to {len(df_resampled)} records at {target_fs}Hz")
        return df_resampled
...
def merge_sensors_for_participant(sensor_dfs: Dict[str, pd.DataFrame], participant: str) -> pd.DataFrame:
    ...
    # Forward fill missing values (reasonable for physiological signals over short gaps)
